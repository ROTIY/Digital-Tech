document.addEventListener('DOMContentLoaded', () => {
  ...
  payButton.addEventListener('click', async () => {
    ...
    alert(`You are about to pay ${amount} KSh for ${games} game${games !== 1 ? 's' : ''} on ${tableId}. Click OK to proceed with M-Pesa payment.`);

    status.textContent = `Requesting payment for ${games} game${games !== 1 ? 's' : ''} on ${tableId}... Check your phone.`;
    payButton.disabled = true;

    try {
      const response = await fetch('/api/initiate-stk', {
        method: 'POST',
